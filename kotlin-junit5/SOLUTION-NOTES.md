...
The TDD examples I have found most useful came with detailed commentary that gave me valuable insights about problem solving, design, refactoring, and testing. The following are some of the authors whose examples have been very helpful in learning about TDD:
...
This is the biggest change we've made so far: a whopping four new lines of code! First, we used `split()` to separate the words in the name. We then formatted the name using a string template.
...
This failing test shows two problems with the current solution.

1. It has the wrong last name (David instead of Thoreau)
2. It doesn't initialize the middle name
...
1. Fix the problem with the last name 
2. Add logic to initialize the middle name 
3. Add the middle initial to the end of the first name
...
This is the most change we've made to the solution so far. 

First, we had to fix the problem with the wrong last name being picked up. The bug was caused by hardcoding `parts[1]` as the last name. This would fail if the name had more than two parts. To fix the bug, replace `parts[1]` with `parts.last()`. For symmetry, replace `parts[0]` with `parts.first()`. 

The logic for initializing the middle name is a bit messy but it works. We'll refactor it for clarity in the next step.
...
1. Added check for one-letter middle name
...
The test fails because the logic to append a suffix is not implemented.
