...
The TDD examples I have found most useful came with detailed commentary that gave me valuable insights into how to approach problem solving, design, refactoring, and testing. Through their examples, folks like Martin Fowler (*Refactoring*), Corey Haines (*Understanding the Four Rules of Simple Design*), and Jeff Langr (*Pragmatic Unit Testing with Java and JUnit*) have all taught me useful new techniques and ways of thinking.

I hope you find something useful here, too, and help me pay it forward.

—Junilu Lacar (April 2025)

## Commit cadence

When you're doing TDD you typically commit every time the tests go green, i.e., they all pass. However, to make it easier to follow this example solution, I committed at each completed step of the TDD cycle, including when the tests went red. At each commit, I added notes to this document to explain the rationale behind the changes in that commit. There will also be a few commits where only commentary was added to this document; those commits will be denoted by a documentation intention (see next section).
...
Before starting TDD, run the existing tests to confirm that the test harness is working. Make this a habit, like how you check your car's dashboard for any warning lights before getting on the road. At the start of this exercise you should see JUnit report nine tests that are disabled/ignored.
...
In these tests, the Zero case is represented by an empty string, the One case represented by a one-word name, the Many case represented by two names, and so forth. The last test is an Exception case, where a name with multiple commas should cause an exception to be thrown. 
...
On the other hand, when we write a test _first_, as we do in TDD, a failing test represents progress. Why? Because the failure means there's functionality that's missing. Rather than going _back_ and fixing what was done, the failing test tells us to move forward and implement the missing functionality.
...
Many people find TDD to be very counterintuitive when they first encounter it. Most programmers are loathe to writing code that is obviously wrong. Solving only for this one specific scenario is not enough and it's hard to put that aside, even just for a little while.
...
3. We _know_ from experience (some would say common sense) that returning a hardcoded value here is wrong, but in TDD, we use a failing test to show that something is wrong rather than reasoning about it with knowledge or intuition.
4. Here, doing the right thing (made the test pass) wrong (returned a hardcoded value) forces us to write another failing test to show that we need to make it better. This is exactly what Dr. Ackoff was talking about.
...
**Result**
    ...
    org.opentest4j.AssertionFailedError at AuthorNameNormalizerTest.kt:17
...
When doing TDD, we use a failing test to show that something is wrong instead of just reasoning about it. The test we enabled focuses on the scenario where a single word name is passed in. Since the current solution only solves for the empty string case, it's not going to work for other types of input. The failing test shows that. This codifies our understanding of the problem and allows us to share that understanding with others.
...
By taking smaller steps, we are practicing emergent design, slowly growing the system and letting it evolve organically.

Commit: `. F make the single word test pass`
...
The expression `name.contains(' ')` is a good candidate for extraction. It's a simple expression but it's not obvious what it's doing. Extracting it to a private function and giving it a name like `isMultiPart()` makes its intent clearer. The conversation, whether it's between members of a pair or mob or in an internal dialog of a solo programmer, might go something like this:

    Verbal info: Does the name contain multiple parts?
    Visual info: name.contains(' ')

    Wait, there's a translation happening from "contains a space" to "is multi-part". We need to refactor that.
...
One nice feature of Kotlin is that it allows you to define extension functions. By defining `isMultiPart()` as an extension function to `String`, we can call it as if it was a method of `name` instead of passing `name` in as a parameter.
...
2. Trimmed `rawName` and assigned the result to a local variable `name`
